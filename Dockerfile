...
RUN set -x && \
    ...
    cd ${OPENSSL} && \
    ./Configure --cross-compile-prefix=${WIN32}- --prefix=/usr/${WIN32} --openssldir=/usr/${WIN32} mingw && \
    make ${MAKEOPTS} && \
    make ${MAKEOPTS} install && \
    make clean && \
    ./Configure --cross-compile-prefix=${WIN64}- --prefix=/usr/${WIN64} --openssldir=/usr/${WIN64} mingw64 && \
    make ${MAKEOPTS} && \
    make ${MAKEOPTS} install && \
    make clean
...
RUN cd /usr/src/${OPENSSL} && \
    ./Configure --cross-compile-prefix=${OSXCROSS}/bin/${OSX32}- --prefix=/usr/${OSX32} --openssldir=/usr/${OSX32}/ darwin-i386-cc && \
    make ${MAKEOPTS} && \
    make ${MAKEOPTS} install && \
    make clean && \
    ./Configure --cross-compile-prefix=${OSXCROSS}/bin/${OSX64}- --prefix=/usr/${OSX64} --openssldir=/usr/${OSX64}/ darwin64-x86_64-cc && \
    make ${MAKEOPTS} && \
    make ${MAKEOPTS} install && \
    make clean 
